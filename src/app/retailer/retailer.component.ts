...
export class RetailerComponent implements OnInit {
  ...
  public addItem()
  {
    ...
    this.billTotal +=  this.selectedItem.retailerPrice * this.itemForm.value['quantity'];
  }
  ...
  remove(item){
    for(var i=0; i< this.customerItems.length; i++)
    {
      ...
      {
        this.billTotal -=  this.customerItems[i].retailerPrice * this.customerItems[i].quantity;
  ...
  changePaymentMethod(){
    document.getElementById("creditPayment").style.visibility = 'hidden';
    if(this.selectedPayment == 'Credit'){
      document.getElementById("creditPayment").style.visibility = 'visible';
      this.remainingAmount = this.billTotal - this.paidAmount;
    }
  }
}
